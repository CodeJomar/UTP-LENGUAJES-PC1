package Peralta_Jomar.scala

import java.time.LocalDate

case class Empleado(nombre: String, apellido: String, edad: Int, puesto: String, salario: Double, fechaContrato: LocalDate)

object AnalisisSalarial {
  ...
  def main(args: Array[String]): Unit = {
    val umbralSalario = 3000.0
    // **Pipeline con funciones de orden superior para filtrar y agrupar**
    val salariosPromedioPorPuesto = empleados
      // Paso 1: Filtra la lista por el salario. El guion bajo '_' es una forma abreviada de la lambda.
      .filter(_.salario > umbralSalario)
      // Paso 2: Agrupa los elementos en un mapa. La clave es el puesto y el valor es una lista de empleados.
      .groupBy(_.puesto)
      // Paso 3: Aplica una función a los valores del mapa (la lista de empleados de cada puesto).
      // La lambda 'listaEmpleados' toma una lista y calcula el promedio.
      .mapValues(listaEmpleados => listaEmpleados.map(_.salario).sum / listaEmpleados.size)

    // Impresión de resultados
    salariosPromedioPorPuesto.foreach {
      case (puesto, promedio) => println(s"Salario promedio de ${puesto}s: $$${"%.2f".format(promedio)}")
    }
  }
